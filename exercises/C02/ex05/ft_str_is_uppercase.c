/*
Similar program compared to lowercase where we use ASCII.
...
int ft_str_is_uppercase(char *str){
    int i=0;
    while(str[i]!='\0'){
        if(!(str[i]>='A' && str[i]<='Z')) //checking if the character in the string has any alphabets not in the range of upper case
        return 0; // checking for not upper case is better and more efficient compared to doing the opposite
        i++;
    }
    return 1;
...
int main(){
    char *str="asdasA";
    printf("Is the string lowercase? : %d \n (1 for yes, 0 for no)", ft_str_is_uppercase(str));
}
